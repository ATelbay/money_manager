 ...
 *
 * Uses JVM PdfBox (org.apache.pdfbox) to create PDF fixtures programmatically and
 * MockK's mockkConstructor to suppress the Android-only PDFBoxResourceLoader.init()
 * call inside PdfTextExtractor.ensureInitialized().
 *
...
class PdfTableExtractorTest {
    ...
    @Before
    fun setUp() {
        // Mock Android Context — PdfTableExtractor holds a reference but only passes it
        // to PdfTextExtractor, which we intercept via mockkConstructor.
        context = mockk(relaxed = true)

        // Intercept PdfTextExtractor construction so ensureInitialized() is a no-op
        // on the JVM (avoids calling PDFBoxResourceLoader.init(context)).
        mockkConstructor(PdfTextExtractor::class)
        every { anyConstructed<PdfTextExtractor>().ensureInitialized() } returns Unit

        extractor = PdfTableExtractor(context)
    }

    @After
    fun tearDown() {
        unmockkConstructor(PdfTextExtractor::class)
    }
